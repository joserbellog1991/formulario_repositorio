...
function guardar_voto() {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data == 1) {
        $("#mensaje").html(
          "<span style='color:green'>Gracias por tu voto</span>"
        );
        document.getElementById("form_voto").reset();
      } else if (data === 0) {
        $("#mensaje").html(
          "<span style='color:red'>Error al ingresar los datos</span>"
        );
      } else if (data == 3) {
        $("#mensaje").html(
          "<span style='color:red'>No pudimos procesar tu solicitud</span>"
        );
        $("#error_rut").html("Rut ya realizó el voto");
        $("#rut").focus();
      } else {
        $("#mensaje").html(
          "<span style='color:red'>No hay respuesta. Intente mas tarde!</span>"
        );
      }
    },
    error: function () {
      // mensaje error
      $("#mensaje").html(
        "<span style='color:red'>Ha ocurrido un error al guardar los datos. Por favor, inténtalo de nuevo más tarde.</span>"
      );
    },
  });
}
